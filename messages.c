...
size_t write_data( void *ptr, size_t size, size_t nmeb, void *stream)
{
	...
	if (curl_output->contents) {
		memcpy(curl_output->contents, ptr, curl_output_size);
...
static void HandleInfoResponse (server_t* server, const char* msg)
{
	...
	if (value == NULL)
	{
		// Server version is lower than 0.3 - Reject
		Com_Printf (MSG_WARNING,
					"> WARNING: rejecting server %s (version <0.3)\n",
					peer_address);
		Sv_Remove(server);
		return;
	}
	...
	// Also get mapname/hostname values -- this might not be a perfect spot, but still it's usable
	value = SearchInfostring (msg, "mapname");
	if (value != NULL)
	{
		strncpy(server->mapname, value, sizeof(server->mapname) - 1);
	}

	value = SearchInfostring (msg, "hostname");
	if (value != NULL)
	{
		strncpy(server->hostname, value, sizeof(server->hostname) - 1);
	}

	// Set a new timeout
	server->timeout = crt_time + TIMEOUT_INFORESPONSE;

	// Write out the new info file
	Sv_WriteInfo();
}
					...
					socket_t recv_socket)
{
	...
	//Heart beat will contain the game and the license
	if (!strncmp (S2M_HEARTBEAT, msg, strlen (S2M_HEARTBEAT)))
	{
		...
		// Extract the game id
		sscanf(msg + strlen(S2M_HEARTBEAT) + 1, "%63s", data);
		Com_Printf (MSG_NORMAL, "> %s ---> heartbeat (%s)\n",
			peer_address, data);

		//Get tokens inside this heart beat
		...
		while (token != NULL)  {
			...
			//Game
			case 0:
				strncpy(gameId, token, sizeof(gameId));
				break;
			//License
			case 1:
				strncpy(license, token, sizeof(license));
				break;
		...
		Com_Printf(MSG_NORMAL, "Game: %s\n", gameId);
		Com_Printf(MSG_NORMAL, "License: %s\n", license);
		...
		strncpy(server->license, license, sizeof (server->license) - 1);
