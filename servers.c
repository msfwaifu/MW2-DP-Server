...
void Sv_WriteInfo (void)
{
	...
	for (ind = 0; ind <= last_used_slot; ind++)
		if (Sv_IsActive(ind))
		{
			...
			Com_PrintInfo("%s,%s,%s,%s,%s,%s,%s,%d\n", sv->license, Sys_SockaddrToString(&sv->address, sv->addrlen), state_string, sv->gamename, sv->gametype, sv->mapname, sv->hostname, sv->challenge_timeout);
		}
